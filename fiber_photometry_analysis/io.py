...
def convert_to_npy(file, **kwargs) :
    ...
    """Function that takes a csv file as an input, extract usefull data from it
    and saves it as a npy file
    
    Args :      file (str) = the path to the csv file
    
    Returns :   npy_file (str) = The path to the npy file
    """
    
    npy_file = os.path.join( os.path.dirname(file), "{0}.npy".format( os.path.basename(file).split(".")[0] ) )
    ...
    np.save(npy_file, sink) #Save the data as numpy file
...
def get_recording_duration_and_sampling_rate(file, allow_downsampling=True) : 
    ...
    photometry_sheet = pd.read_csv( file, header=1, usecols=[0] ) #Load the data
