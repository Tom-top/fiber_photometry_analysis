...
def live_video_plot(video_clip, x_data, y_data, **kwargs):
    ...
    def make_frame_mpl(t):
        ...
        if i < kwargs["video_photometry"]["display_threshold"]*kwargs["video_photometry"]["plot_acceleration"]:
            behavior_plot.set_data(x_data.iloc[0:i], y_data[0][0:i])
            df_plot.set_data(x_data.iloc[0:i], y_data[1][0:i])
            ...
            live_ax0.set_xlim(x_data.iloc[0]+delta, x_data.iloc[0]+(i/kwargs["video_photometry"]["plot_acceleration"]))
            live_ax1.set_xlim(x_data.iloc[0]+delta, x_data.iloc[0]+(i/kwargs["video_photometry"]["plot_acceleration"]))
    ...
    live_ax0.set_xlim([x_data.iloc[0], x_data.iloc[0]+kwargs["video_photometry"]["display_threshold"]])
    live_ax0.set_yticks([0, 1])
    live_ax0.set_yticklabels(["Not behaving", "Behaving"], fontsize=10.)
    live_ax0.set_ylim(-0.1, 1.1)
    
    behavior_plot, = live_ax0.plot(x_data.iloc[0], y_data[0][0], '-', color="red", alpha=0.8, ms=1., lw=3.)
    ...
    live_ax1.set_xlim([x_data.iloc[0], x_data.iloc[0]+kwargs["video_photometry"]["display_threshold"]])
    ...
    df_plot, = live_ax1.plot(x_data.iloc[0], y_data[1][0], '-', color="green", alpha=1., ms=1., lw=3.)
    
    plt.tight_layout()

    video_duration = video_clip.duration
    animation = mpy.VideoClip(make_frame_mpl, duration=(video_duration * kwargs["video_photometry"]["plot_acceleration"]))
    ...
    # final_clip = mpy.concatenate_videoclips(final_clip,
    #                                         method = "compose")
